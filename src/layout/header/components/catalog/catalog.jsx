...
export const Catalog = ({onClose}) => {
    ...
    return (
        <div className={`border-2 p-10 flex nax-sm:max-h-[300px] max-sm:h-full  max-sm:overflow-y-auto max-sm:overflow-hidden  max-sm:p-5 gap-8  flex-wrap bg-white items-center max-sm:gap-1 max-sm:flex-col justify-center  rounded-md `} >
            {data?.map((item) => (
                <Link key={item.name} className='max-sm:w-full' onClick={submit} to={`/category/${item.name}`} >
